...
from page_home_financing import HomeFinancingPage
import math
import pytest
import time

URL = 'https://cd-sit.meritageweb.dev/buyer-resources/home-financing'
...
class Test_Home_Financing_Page(BasicTest):
  @pytest.fixture()
  ...
  def test_article_containers(self, number):
    ...
    article_bool = False
    if 'Article' or 'Resource' == article_element_text:
      article_bool = True
    ...
    if article_element_text == 'Resource':
      header_bool = True
    else:
  ...
  def test_article_3(self):
    ...
    final_bool = False
    if 'Bathroom design trends' in header and 'Explore bathroom design' in body:
      final_bool = True
    home_page.click_element_related_article_button(2) # Button does not do anything
    assert (
      final_bool and
      'Bathroom' == self.driver.title
    )
